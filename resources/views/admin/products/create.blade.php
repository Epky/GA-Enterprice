<x-admin-layout>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <x-searchable-select
                                        ...
                                        :required="true"
                                        deleteRoute="admin.categories.delete-inline"
                                        refreshRoute="admin.categories.active"
                                ...
                                <div>
                                    <x-searchable-select
                                        ...
                                        :required="true"
                                        deleteRoute="admin.brands.delete-inline"
                                        refreshRoute="admin.brands.active"
    ...
    <x-inline-add-modal 
        ...
        storeRoute="admin.categories.store-inline"
        :fields="[
    ...
    <x-inline-add-modal 
        id="brand-modal"
        title="Add New Brand"
        storeRoute="admin.brands.store-inline"
        :fields="[
    ...
    <script>
        ...
        function populateReview() {
            ...
            // Basic Information
            reviewHtml += '<div class="border-b border-gray-200 pb-4">';
            reviewHtml += '<h4 class="text-md font-semibold text-gray-900 mb-3">Basic Information</h4>';
            reviewHtml += '<dl class="grid grid-cols-1 md:grid-cols-2 gap-4">';
            
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Product Name</dt><dd class="mt-1 text-sm text-gray-900">' + (formData.get('name') || 'N/A') + '</dd></div>';
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">SKU</dt><dd class="mt-1 text-sm text-gray-900">' + (formData.get('sku') || 'N/A') + '</dd></div>';
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Price</dt><dd class="mt-1 text-sm text-gray-900">₱' + (formData.get('base_price') || '0.00') + '</dd></div>';
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Status</dt><dd class="mt-1 text-sm text-gray-900">' + (formData.get('status') || 'active') + '</dd></div>';
            
            // Get category and brand names
            const categorySelect = document.getElementById('category_id');
            const brandSelect = document.getElementById('brand_id');
            const categoryName = categorySelect ? categorySelect.options[categorySelect.selectedIndex]?.text : 'N/A';
            const brandName = brandSelect ? brandSelect.options[brandSelect.selectedIndex]?.text : 'N/A';
            
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Category</dt><dd class="mt-1 text-sm text-gray-900">' + categoryName + '</dd></div>';
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Brand</dt><dd class="mt-1 text-sm text-gray-900">' + brandName + '</dd></div>';
            reviewHtml += '<div class="md:col-span-2"><dt class="text-sm font-medium text-gray-500">Description</dt><dd class="mt-1 text-sm text-gray-900">' + (formData.get('description') || 'N/A') + '</dd></div>';
            reviewHtml += '<div><dt class="text-sm font-medium text-gray-500">Featured</dt><dd class="mt-1 text-sm text-gray-900">' + (formData.get('is_featured') ? 'Yes' : 'No') + '</dd></div>';
            
            reviewHtml += '</dl></div>';
            
            // Images
            reviewHtml += '<div class="border-b border-gray-200 pb-4">';
            reviewHtml += '<h4 class="text-md font-semibold text-gray-900 mb-3">Images</h4>';
            const imageFiles = formData.getAll('images[]');
            if (imageFiles && imageFiles.length > 0 && imageFiles[0].size > 0) {
                reviewHtml += '<p class="text-sm text-gray-900">' + imageFiles.length + ' image(s) selected</p>';
            } else {
                reviewHtml += '<p class="text-sm text-gray-500">No images uploaded</p>';
            }
            reviewHtml += '</div>';
            
            // Variants
            reviewHtml += '<div>';
            reviewHtml += '<h4 class="text-md font-semibold text-gray-900 mb-3">Variants</h4>';
            const variantItems = document.querySelectorAll('.variant-item');
            if (variantItems.length > 0) {
                reviewHtml += '<div class="space-y-3">';
                variantItems.forEach((item, index) => {
                    const variantNum = item.getAttribute('data-variant');
                    const type = formData.get(`variants[${variantNum}][variant_type]`) || 'N/A';
                    const value = formData.get(`variants[${variantNum}][variant_value]`) || 'N/A';
                    const sku = formData.get(`variants[${variantNum}][sku]`) || 'N/A';
                    const priceAdj = formData.get(`variants[${variantNum}][price_adjustment]`) || '0.00';
                    const stock = formData.get(`variants[${variantNum}][stock_quantity]`) || '0';
                    
                    reviewHtml += '<div class="bg-gray-50 p-3 rounded">';
                    reviewHtml += '<p class="text-sm font-medium text-gray-900">Variant ' + (index + 1) + '</p>';
                    reviewHtml += '<p class="text-sm text-gray-600">Type: ' + type + ' | Value: ' + value + ' | SKU: ' + sku + '</p>';
                    reviewHtml += '<p class="text-sm text-gray-600">Price Adjustment: ₱' + priceAdj + ' | Stock: ' + stock + '</p>';
                    reviewHtml += '</div>';
                });
                reviewHtml += '</div>';
            } else {
                reviewHtml += '<p class="text-sm text-gray-500">No variants added</p>';
            }
            reviewHtml += '</div>';
