...
<body>
        ...
        <div class="content-page">
            <div class="container-fluid">
                                ...
                                <div class="card">
                                    <div class="card-body d-flex">
                                        <div class="product-image">
                                            <input type="checkbox" class="item-checkbox" data-order-id="<?php echo htmlspecialchars($row['order_id']); ?>" data-subtotal="<?php echo $subtotal; ?>" checked>
                                            <img src="../uploads/<?php echo htmlspecialchars($row['product_image_1']); ?>" class="img-thumbnail ml-2" alt="Product Image">
                                        </div>
                ...
                <div class="text-right mt-4">
                    ...
                    <a href="javascript:void(0)" class="btn btn-success mt-2 proceed-to-checkout">Proceed to Checkout</a>
                    <br>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.quantity').on('change', function() {
                ...
                $.post('cart.php', {
                    ...
                    if (data.status === 'success') {
                        ...
                        // Update the subtotal data attribute in the checkbox
                        $productDetails.closest('.card').find('.item-checkbox').data('subtotal', newSubtotal);
            ...
            // Handle checkbox selection for subtotal calculation
            $('.item-checkbox').on('change', function() {
                recalculateTotalPrice();
            });

            // Function to recalculate the total price of selected items
            function recalculateTotalPrice() {
                var totalPrice = 0;
                $('.item-checkbox:checked').each(function() {
                    var subtotalValue = parseFloat($(this).data('subtotal'));
        ...
        $(document).ready(function() {
            ...
            $('.proceed-to-checkout').click(function() {
                ...
                $('.item-checkbox:checked').each(function() {
                    var $productDetails = $(this).closest('.card-body');
                    var orderId = $(this).data('order-id');
                    var productId = $productDetails.find('.quantity').data('product-id');
                    var itemQuantity = $productDetails.find('.quantity').val();

                    // Add the collected data to the selectedItems array
                    selectedItems.push({
                        order_id: orderId,
                        product_id: productId,
                        item_quantity: itemQuantity
                ...
                // Send selected items to session via AJAX
                $.post('set_selected_items.php', {
                    ...
                    if (data.status === 'success') {
                        ...
                        alert('Error saving selected items. Please try again.');
                    }
                });
            });
        });
